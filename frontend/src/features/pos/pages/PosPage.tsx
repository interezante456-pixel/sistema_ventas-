...
export const PosPage = () => {
  ...
  // ... otros estados ...
  const [lastSale, setLastSale] = useState<any>(null); // Guardará la venta recién hecha
  const [isSuccessOpen, setIsSuccessOpen] = useState(false); // Controla el modal verde
  ...
  const addToCart = (product: any) => {
    ...
    if (currentQty + 1 > product.stock) {
      setToast({
        msg: `¡Stock insuficiente! Solo quedan ${product.stock}`,
        type: "error",
      });
      return;
    }
  ...
  const handleProcessSale = async (paymentData: any) => {
    try {
      ...
      // 2. RECIBIR LA RESPUESTA COMPLETA (backend paso 1)
      const createdSale = response.data;

      // 3. Agregar el dato "montoPago" manualmente para el cálculo de vuelto en el modal
      // (Ya que el backend a veces no devuelve el monto que entregó el cliente, solo el total)
      createdSale.montoPago = paymentData.montoPago;

      // 4. Guardar venta y ABRIR MODAL DE ÉXITO 🟢
      setLastSale(createdSale);
      setIsPaymentOpen(false); // Cierra modal de pago
      setIsSuccessOpen(true); // Abre modal de éxito

      // 5. Limpiar carrito y recargar productos
      setCart([]);
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filteredProducts.map((product) => (
                ...
                >
                  {/* Stock Badge */}
                  <span
                    className={`absolute top-2 right-2 px-2 py-0.5 text-[10px] font-bold rounded-full z-10 ${
                      product.stock <= 5
